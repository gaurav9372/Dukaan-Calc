...
const getCachedInputStyles = (input) => {
  if (input._cachedGroup === undefined) {
    const group = input.closest(".input-group");
    input._cachedGroup = group || null;
    if (group) {
      input._cachedUnit = group.querySelector(".unit") || null;
      input._cachedPaddingLeft = parseFloat(window.getComputedStyle(group).paddingLeft) || 0;
    } else {
      input._cachedUnit = null;
      input._cachedPaddingLeft = 0;
    }
