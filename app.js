...
const updateCaret = (input) => {
  // ⚡ Bolt: Cache DOM lookups to prevent layout thrashing on every keypress
  // Impact: Eliminates redundant `.closest()` and `.querySelector()` calls
  ...
  if (group._cachedUnit === undefined) {
    group._cachedUnit = group.querySelector(".unit");
  }
  const unit = group._cachedUnit;

  // ⚡ Bolt: Cache computed styles to prevent forced synchronous layout
  // Impact: Eliminates expensive getComputedStyle() recalculations during typing
...
const invalidateCaches = () => {
  // ⚡ Bolt: Clear cached properties on resize to ensure correct caret positioning
  document.querySelectorAll("input, .input-group").forEach((el) => {
